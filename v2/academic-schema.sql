...
CREATE TABLE IF NOT EXISTS diagnostic_item_localizations (
  ...
  entry_stage TEXT NOT NULL DEFAULT 'foundations' CHECK(entry_stage IN ('foundations','development')),
  entry_basis TEXT NOT NULL DEFAULT 'standard' CHECK(entry_basis IN ('standard','rating_seed','coach')),
...
CREATE TABLE IF NOT EXISTS academic_external_games (
  id TEXT PRIMARY KEY,
  student_id TEXT NOT NULL REFERENCES students(id) ON DELETE CASCADE,
  account_id TEXT NOT NULL REFERENCES player_accounts(id) ON DELETE CASCADE,
  platform TEXT NOT NULL CHECK(platform IN ('lichess','chesscom')),
  ...
  url TEXT,
  played_at TEXT,
  student_color TEXT CHECK(student_color IN ('white','black')),
  student_result TEXT CHECK(student_result IN ('win','loss','draw','unknown')),
  ...
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(account_id,external_game_id)
);
...
CREATE TABLE IF NOT EXISTS cis_bot_profiles (
  ...
  skill_focus_json TEXT NOT NULL DEFAULT '[]', active INTEGER NOT NULL DEFAULT 1 CHECK(active IN (0,1))
);
CREATE TABLE IF NOT EXISTS cis_bot_challenges (
  ...
  bot_id TEXT NOT NULL REFERENCES cis_bot_profiles(id) ON DELETE RESTRICT,
  status TEXT NOT NULL DEFAULT 'in_progress' CHECK(status IN ('in_progress','passed','failed','abandoned')),
  games_required INTEGER NOT NULL DEFAULT 3, points REAL NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS cis_bot_games (
  id TEXT PRIMARY KEY, challenge_id TEXT NOT NULL REFERENCES cis_bot_challenges(id) ON DELETE CASCADE,
  game_no INTEGER NOT NULL, student_color TEXT NOT NULL CHECK(student_color IN ('white','black')),
  status TEXT NOT NULL DEFAULT 'in_progress' CHECK(status IN ('in_progress','completed','abandoned')),
  fen TEXT NOT NULL, moves_uci TEXT NOT NULL DEFAULT '', result TEXT, pgn TEXT, analysis_json TEXT NOT NULL DEFAULT '{}',
  started_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, completed_at TEXT, UNIQUE(challenge_id,game_no)
);
...
CREATE TABLE IF NOT EXISTS practice_bank_puzzles (
...
CREATE TABLE IF NOT EXISTS practice_streak_runs (
  id TEXT PRIMARY KEY,student_id TEXT NOT NULL REFERENCES students(id) ON DELETE CASCADE,
  status TEXT NOT NULL DEFAULT 'in_progress' CHECK(status IN ('in_progress','completed','abandoned')),
...
CREATE TABLE IF NOT EXISTS practice_streak_items (
  run_id TEXT NOT NULL REFERENCES practice_streak_runs(id) ON DELETE CASCADE,
  sequence_no INTEGER NOT NULL,puzzle_id TEXT NOT NULL REFERENCES practice_bank_puzzles(id) ON DELETE CASCADE,
  attempt_id TEXT REFERENCES practice_bank_attempts(id) ON DELETE SET NULL,correct INTEGER CHECK(correct IN (0,1)),
...
CREATE TABLE IF NOT EXISTS practice_storm_runs (
  id TEXT PRIMARY KEY,student_id TEXT NOT NULL REFERENCES students(id) ON DELETE CASCADE,
  status TEXT NOT NULL DEFAULT 'in_progress' CHECK(status IN ('in_progress','completed','abandoned')),
