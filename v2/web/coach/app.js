...
async function openStudent(studentId){
  activeStudentId=studentId; const [d,nextData,learning,curriculum,ratings,training,openingTrainer,nextEngine]=await Promise.all([api(`../api/admin/students/${encodeURIComponent(studentId)}/profile`),api(`../api/admin/students/${encodeURIComponent(studentId)}/next-lesson`),api(`../api/admin/students/${encodeURIComponent(studentId)}/learning-priorities`),api('../api/admin/curriculum/hmena'),api(`../api/admin/students/${encodeURIComponent(studentId)}/rating-progress`),api(`../api/admin/students/${encodeURIComponent(studentId)}/training-intelligence`),api(`../api/admin/students/${encodeURIComponent(studentId)}/opening-trainer`),api(`../api/admin/students/${encodeURIComponent(studentId)}/next-lesson-engine?locale=es`)]);
  ...
  const lessonContent=nextEngine.recommendation?.lesson?.content||{};const lessonPlanDetails=nextEngine.recommendation?.lesson?`<div class="lesson-plan-mini"><h4>Plan rápido de clase</h4>${lessonContent.keyPoints?.length?`<ul>${lessonContent.keyPoints.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:''}${lessonContent.activity?`<p><strong>Actividad:</strong> ${esc(lessonContent.activity)}</p>`:''}${lessonContent.homework?`<p><strong>Tarea:</strong> ${esc(lessonContent.homework)}</p>`:''}${lessonContent.exitTicket?`<p><strong>Exit ticket:</strong> ${esc(lessonContent.exitTicket)}</p>`:''}</div>`:'';
