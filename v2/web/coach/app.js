...
async function openStudent(studentId){
  ...
  const practiceMissions=(d.assignments||[]).map(a=>({...a,meta:safeJson(a.details)})).filter(a=>a.meta.kind==='external_practice');
  const practiceRows=practiceMissions.length?practiceMissions.map(a=>`<li><strong>${esc(a.title)}</strong> · <span class="status-badge ${a.status==='completed'?'status-ok':a.status==='submitted'?'status-warn':''}">${esc(a.status)}</span>${a.meta.url?` · <a href="${esc(a.meta.url)}" target="_blank" rel="noopener noreferrer">Lichess ↗</a>`:''}${a.status==='submitted'?` <button class="btn btn-secondary complete-practice-coach" data-assignment-id="${esc(a.id)}" type="button">Confirmar completado</button>`:''}</li>`).join(''):'<li>Sin misiones externas asignadas.</li>';
...
document.addEventListener('click',async event=>{
  const completePractice=event.target.closest('.complete-practice-coach');if(completePractice){try{await api(`../api/admin/practice-missions/${encodeURIComponent(completePractice.dataset.assignmentId)}/status`,{method:'PUT',body:JSON.stringify({status:'completed'})});await openStudent(activeStudentId);}catch(e){$('#error').textContent=e.message;}return;}
