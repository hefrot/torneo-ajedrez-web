...
function assessmentCta(s){
  ...
  const level=band==='hmena-800-1200'?'1200':'0800',title=level==='1200'?t('diagnostic1200'):t('assessmentMission');
...
function studentCard(s){
  ...
  const ratingLabel=rating?`${rating.latestRating}`:'—',ratingMeta=rating?`${rating.platform} ${rating.ratingType} · ${rating.points?.[0]?.gamesCount??'—'} ${t('experience')}`:t('noRatings');
