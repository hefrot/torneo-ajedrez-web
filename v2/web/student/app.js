...
function practiceBankBlock(s){
  ...
  return `<div class="tactical-lab-head"><div><strong>${t('tacticalLab')}</strong><small>${t('tacticalLabIntro')}</small></div><span>${bank.summary?.correct7d||0} ${t('correctThisWeek')}</span></div><div class="puzzle-dashboard"><article><strong>${p.rating??'—'}</strong><span>${t('puzzleRating')}</span></article><article><strong>${accuracy}</strong><span>${t('accuracy')}</span></article><article><strong>${p.streak??0}</strong><span>${t('streak')}</span></article><article><strong>${streak.best??0}</strong><span>${t('streakBest')}</span></article></div><button class="btn btn-secondary start-puzzle-streak" data-student-id="${esc(s.id)}" type="button">${t('puzzleStreak')} · ${streak.activeRunId?`${t('streakScore')} ${streak.current}`:t('startStreak')}</button>${details}${themeDetail}<div class="puzzle-grid tactical-bank-grid">${cards}</div><small class="practice-source-note">${t('cc0Source')} <a href="https://database.lichess.org/#puzzles" target="_blank" rel="noopener noreferrer">Lichess</a></small>`;
}
