...
function practiceBankBoard(p,studentId){
  ...
  return `<article class="mistake-puzzle bank-puzzle" data-student-id="${esc(studentId)}" data-puzzle-id="${esc(p.id)}" data-turn="${esc(turn)}"><div class="puzzle-meta"><strong>${esc(theme.replace(/([A-Z])/g,' $1'))}</strong><small>${t('puzzleInstruction')}</small></div><div class="puzzle-board">${squares.join('')}</div><small class="puzzle-help">${t('selectMove')} · ${locale==='es'?'Dificultad':'Difficulty'} ${p.rating??'—'}</small><div class="puzzle-result" aria-live="polite"></div></article>`;
}
...
function studentCard(s){
  ...
  return `<article class="panel student-progress-card family-dashboard">
    ...
    <section class="family-section practice-lab" id="practice-${esc(s.id)}"><div class="family-section-head"><div><span>${t('practiceLab')}</span><h3>${t('practiceIntro')}</h3></div><small>${t('trackingNote')}</small></div>${practiceBankBlock(s)}<h4>${t('externalPractice')}</h4><div class="practice-grid">${practiceMissionCards(s)}</div><h4>${t('mistakePuzzles')}</h4><div class="puzzle-grid">${puzzles}</div></section>
