...
Object.assign(copy.en,{tacticalLab:'CIS Tactical Lab',tacticalLabIntro:'Verified puzzles selected from current training priorities.',correctThisWeek:'correct this week',cc0Source:'Puzzle source: Lichess CC0 database.'});
Object.assign(copy.es,{tacticalLab:'Laboratorio táctico CIS',tacticalLabIntro:'Problemas verificables seleccionados desde las prioridades actuales.',correctThisWeek:'correctos esta semana',cc0Source:'Fuente de problemas: base CC0 de Lichess.'});
...
document.addEventListener('click',async event=>{
  ...
  const bankSquare=event.target.closest('.bank-square');if(bankSquare){
    ...
    try{const result=await api(`../api/portal/students/${encodeURIComponent(card.dataset.studentId)}/practice-bank/${encodeURIComponent(card.dataset.puzzleId)}/attempt`,{method:'POST',body:JSON.stringify({answerMove})});const msg=card.querySelector('.puzzle-result');if(result.correct){msg.textContent=t('correctMove');card.classList.add('solved');setTimeout(()=>loadPortal().catch(()=>{}),650);}else msg.textContent=t('tryAgain');}catch(e){$('#portal-error').textContent=e.message;}return;
  }
