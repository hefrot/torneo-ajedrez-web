...
test('Smayan-like pilot flows from reliable Chess.com rating through assessment to training',()=>{
  ...
  seedAllCurriculum(db);seedHmenaFramework(db);mapLegacyToHmena(db);seedDiagnostic0800(db);seedCisBots(db);
  ...
  const journey=familyJourney(db,student.id,{locale:'en'});assert.equal(journey.mission.kind,'practice');assert.equal(journey.completed,1);
  const catalog=cisBotCatalog(db,student.id,{locale:'en'});assert.equal(catalog.recommendedCode,'queen-tactician');
