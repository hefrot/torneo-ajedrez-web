...
function persistAnalysis(db,game,analysis){
  ...
  for(const item of analysis.critical||[]){
    ...
    upsert.run({id,studentId:game.student_id,sourceGameId:game.external_game_id,skillId,findingType:item.findingType||'engine_mistake',severity:item.severity||1,cpLoss:item.cpLoss??null,confidence:item.classifierConfidence??null,classifierSource:analysis.analysisVersion||'academic-stockfish-v1',ply:item.ply??null,moveNumber:item.moveNumber??null,fenBefore:item.fenBefore||null,movePlayed:item.movePlayedUci||null,bestMove:item.bestMoveUci||null,note:item.movePlayedSan&&item.bestMoveSan?`${item.movePlayedSan} → ${item.bestMoveSan}`:null});findings++;
  }
