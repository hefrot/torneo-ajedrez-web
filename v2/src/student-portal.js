import {getHmenaPlacement,recommendLearningPriorities} from './hmena-curriculum.js';
import {studentRatingProgress} from './rating-tracking.js';

export function studentPortalDashboard(db,accountId,{now=new Date()}={}){
  const account=db.prepare("SELECT id,login_name AS loginName,display_name AS displayName,role FROM portal_accounts WHERE id=? AND status='active'").get(accountId);
  if(!account)return null;
  const students=db.prepare(`SELECT s.id,s.display_name AS displayName,s.status,s.current_level AS currentLevel,s.target_level AS targetLevel FROM portal_account_students pas JOIN students s ON s.id=pas.student_id WHERE pas.account_id=? ORDER BY s.display_name`).all(accountId);
  const upcomingStmt=db.prepare(`SELECT cs.id,cs.starts_at AS startsAt,cs.duration_minutes AS durationMinutes,cs.week_no AS weekNo,cs.title,p.name AS programName,sc.name AS schoolName FROM enrollments e JOIN class_sessions cs ON cs.program_id=e.program_id JOIN programs p ON p.id=e.program_id LEFT JOIN schools sc ON sc.id=p.school_id WHERE e.student_id=? AND e.status='active' AND cs.status='scheduled' AND cs.starts_at>=? ORDER BY cs.starts_at LIMIT 8`);
  ...
  return {account,students:students.map(student=>({
    ...student,
    placement:getHmenaPlacement(db,student.id),
    priorities:recommendLearningPriorities(db,student.id,{limit:5}).priorities,
    ...
    skills:skillStmt.all(student.id),
