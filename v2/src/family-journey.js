const copy={
  en:{
    ...
    assessmentContinue:['Continue level check','You cleared the first screening. Complete the 800–1200 check to continue finding your starting level.'],
    assessmentAdvanced:['Continue advanced level check','You cleared the lower screening. Continue through the advanced bands until CIS finds the first band that needs reinforcement.'],
  ...
  es:{
    ...
    assessmentContinue:['Continuar evaluación','Superaste el primer filtro. Completa la evaluación 800–1200 para seguir encontrando tu nivel inicial.'],
    assessmentAdvanced:['Continuar evaluación avanzada','Superaste el filtro inferior. Continúa por las bandas avanzadas hasta encontrar la primera que necesite refuerzo.'],
...
export function familyJourney(db,studentId,{locale='en'}={}){
  ...
  const advancedBlueprintByBand={'hmena-1200-1600':'DIAG-HMENA-1200-1600','hmena-1600-2000':'DIAG-HMENA-1600-2000','hmena-2000-2300':'DIAG-HMENA-2000-2300','hmena-2300-2500':'DIAG-HMENA-2300-2500'};
  const advancedBlueprint=placement?.source==='assessment'?advancedBlueprintByBand[placement?.code]||null:null;
  const continueAdvanced=Boolean(advancedBlueprint&&!exists("SELECT 1 FROM diagnostic_attempts WHERE student_id=? AND blueprint_id=? AND status='completed' LIMIT 1")(studentId,advancedBlueprint));
  const assessment=(anyCompletedDiagnostic||Boolean(placement))&&!continue1200&&!continueAdvanced;
  ...
  const mission=current==='done'
    ...
    :current==='waitCoach'
      ...
      :current==='assessment'&&continue1200
        ...
        :current==='assessment'&&continueAdvanced
          ?{kind:'assessment',title:copy[l].assessmentAdvanced[0],description:copy[l].assessmentAdvanced[1],diagnosticLevel:'advanced'}
