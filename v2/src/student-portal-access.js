...
export function createPortalAccount(db,{studentIds=[],role='guardian',displayName,loginName=null,ttlDays=365}={}){
  ...
  db.transaction(()=>{
    db.prepare('INSERT INTO portal_accounts(id,login_name,display_name,role) VALUES (?,?,?,?)').run(accountId,login,String(displayName||'').trim()||login,role);
  ...
  return {accountId,loginName:login,accessCode:code,expiresAt,warning:'El código se muestra una sola vez.'};
}
export function loginPortalAccount(db,{loginName,accessCode:code,now=new Date(),sessionDays=30}={}){
  const account=db.prepare("SELECT id,login_name,display_name,role FROM portal_accounts WHERE lower(login_name)=lower(?) AND status='active'").get(String(loginName||'').trim());
  ...
  return {sessionToken:token,expiresAt,account:{id:account.id,loginName:account.login_name,displayName:account.display_name,role:account.role}};
}
...
export function authenticatePortalSession(db,token,{now=new Date(),touch=true}={}){
  ...
  const supplied=digest(token),rows=db.prepare(`SELECT ps.id,ps.account_id,ps.token_hash,ps.expires_at,pa.role,pa.display_name FROM portal_sessions ps JOIN portal_accounts pa ON pa.id=ps.account_id WHERE ps.revoked_at IS NULL AND pa.status='active'`).all();
  ...
  return {accountId:row.account_id,role:row.role,displayName:row.display_name,sessionId:row.id};
}
