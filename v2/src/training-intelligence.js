...
export function studentTrainingIntelligence(db,studentId,{locale='en',includeTechnical=false}={}){
  ...
  const reviewedGames=Number(db.prepare("SELECT COUNT(*) AS n FROM student_game_reviews WHERE student_id=?").get(studentId)?.n||0);
